...
public class SonatypeNx3Publisher extends ArtifactStorePublisherSupport {
    ...
    private Map<String, List<Artifact>> groupArtifactsByGav(ArtifactStore artifactStore) {
        ...
        for (Artifact artifact : artifactStore.artifacts()) {
            // Skip metadata files
            if (artifact.getExtension().equals("xml")
                    && artifact.getArtifactId().equals("maven-metadata")) {
                continue;
            }

            // Skip checksum files (they're auto-generated by NXRM3)
            if (artifact.getExtension().equals("md5")
                    || artifact.getExtension().equals("sha1")
                    || artifact.getExtension().equals("sha256")
                    || artifact.getExtension().equals("sha512")) {
                continue;
            }
